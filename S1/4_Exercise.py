...
class AnalizadorDefectos:
    ...
    def generar_reporte(self):
        ...
        for modulo, defectos in self.defectos_por_modulo.items():
            total = len(defectos)
            criticos = sum(1 for s in defectos if s == "Crítico")
            pct = (criticos / total * 100) if total else 0.0

            print(f"\n🔹 {modulo}:")
            print(f"   Total: {total} defectos")
            print(f"   Críticos: {criticos} ({pct:.1f}%)")

            # Principio de Pareto: 80% de problemas en 20% del código
            if total and (criticos / total) > 0.3:
                print("   ⚠️  MÓDULO DE ALTO RIESGO")
